...
class MerkleTree:
    ...
    def _get_proof_helper(self, current_node: MerkleNode) -> str:
        ...
        brother = "0" + current_node.parent.left.digest \
    ...
    def get_proof(self, index: int) -> str:
        if not self.root or index >= len(self.leafs) or index < 0:
            return ''
        return self.root.digest + self._get_proof_helper(self.leafs[index])

    # input 4 - Proof of Inclusion
    def _check_proof_helper(value, digest_list) -> str:
        if not digest_list:
            return value
        ...
        return MerkleTree._check_proof_helper(value, digest_list[1:])

    def check_proof(proof: str) -> str:
        proof = proof.split()
        value = proof[0]
        digest_list = proof[2:]
        return MerkleTree._check_proof_helper(get_hexdigest(value.encode()), digest_list) == proof[1]
...
class SparseTree:
    ...
    def __init__(self):
        self.changed_leafs: list[SparseNode] = []
        self.default_values = []
    ...
    def change_leaf_value(self, digest):
        ...
        if SparseTree.is_contain(index, self.changed_leafs):
            return
        # change the leaf in index
        self.changed_leafs.append(SparseNode(value="1", index=index))

    # input 9 - Change Value
    def calc_root_helper(self) -> int:
        level = self.changed_leafs.copy()
        level_above = []
        for i in range(256):
    ...
    def get_proof_helper(self, index, level, i):
        proof = ""
        for i in range(256):
    ...
    def get_proof(self, digest):
        ...
        index = int(digest, 16)
        level = self.changed_leafs.copy()
        proof = self.get_proof_helper(index, level, 0)
        return root_val + ' ' + proof
    ...
    def calc_index_by_len_proof(len: int, index: int) -> int:
        for i in range(256 + 2 - len):
            index = math.floor(index >> 1)
        return index
...
def main_function():
    ...
    while True:
        ...
        if (0 < func_number) and (func_number < 8):
            if func_number == 4:
                print(MerkleTree.check_proof(command[1]))
            t.set_command(func_number, command)
        elif (7 < func_number) and (func_number < 12):
            if func_number == 11:
